...
contract('SplitPayment', (accounts) => {
  ...
  it('Should NOT split payment if array lenth mismatch', async () => {
    const recipients = [accounts[1], accounts[2], accounts[3]];
    const amounts = [40, 20];
    try {
      await splitPayment.send(recipients, amounts, {
      ...
      assert(
        error.message.includes('to and amount arrays must have same length')
      );
      return;
    }
    assert(false);
  ...
  it('Should NOT split payment if callerr is not owner', async () => {
    const recipients = [accounts[1], accounts[2], accounts[3]];
    const amounts = [40, 20, 30];
    ...
    try {
      ...
      assert(error.message.includes('only owner can send transactions'));
      return;
    }
    assert(false);
  });
